...
CREATE OR REPLACE FUNCTION private.is_builders_enabled(_restaurant_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION private.is_builders_enabled(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION private.is_builders_enabled(uuid) TO anon, authenticated;
...
CREATE POLICY "Public can view active builders" ON public.builders
  FOR SELECT
  USING (is_active = true AND private.is_builders_enabled(restaurant_id));
...
CREATE POLICY "Public can view groups" ON public.builder_groups
  ...
  USING (EXISTS (
    ...
    WHERE b.id = builder_groups.builder_id
      ...
      AND private.is_builders_enabled(b.restaurant_id)
  ));
...
DROP FUNCTION IF EXISTS public.is_builders_enabled(uuid);
