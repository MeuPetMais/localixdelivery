-- Fix service-role detection for internal webhook/system order transitions.
--
-- Supabase/PostgREST currently supplies the role inside request.jwt.claims;
-- request.jwt.claim.role may be NULL. auth.role() handles both layouts.
-- This migration intentionally recreates only order_apply_transition with
-- the same signature and business rules. No grants are changed.
