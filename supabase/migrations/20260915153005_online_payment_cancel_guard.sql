-- Guard direct cancellation of already-approved online Mercado Pago orders.
--
-- Refund orchestration remains in the application/Edge Function path
-- (cancelRestaurantOrder -> mp-payment-intent:refund). The central transition
-- RPC must fail closed when a caller tries to bypass that path.
