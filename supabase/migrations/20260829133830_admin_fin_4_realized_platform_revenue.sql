-- ADMIN-FIN-4: recognize Localix realized platform revenue on operational
-- realization, inside the canonical order transition RPC.
--
-- Rules:
-- - entregue/concluido: realized_platform_revenue = platform_revenue
-- - reembolsado/chargeback: realized_platform_revenue = 0
-- - no recalculation, no frontend input, no hardcoded fee.
-- - missing snapshot must not block the operational transition.
