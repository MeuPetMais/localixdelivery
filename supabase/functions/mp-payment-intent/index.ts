...
async function getAccessToken(sb, restaurantId) {
  const { data } = await sb.from("mercado_pago_accounts").select("access_token, connected").eq("restaurant_id", restaurantId).maybeSingle();
...
function sanitizeMpCause(cause) {
  ...
  return cause.filter((item)=>!!item && typeof item === "object").map((item)=>{
    const sanitized = {};
...
function getPayloadCard(payload) {
  ...
  if (!Number.isInteger(installments) || installments <= 0) throw new Error("card_installments_required");
  const payer = card.payer && typeof card.payer === "object" ? card.payer : {};
  const identificationType = String(payer.identification_type ?? payer.identificationType ?? "").trim();
  const identificationNumber = String(payer.identification_number ?? payer.identificationNumber ?? "").replace(/\D/g, "");
  return {
    token,
    paymentMethodId,
    installments,
    issuerId: issuerIdRaw && /^\d+$/.test(issuerIdRaw) ? issuerIdRaw : null,
    payerIdentification: identificationType && identificationNumber ? {
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: order, error: ordErr } = await sb.from("orders").select("id, restaurant_id, order_number, total, customer_id, customer_name, customer_phone, address, items").eq("id", orderId).maybeSingle();
    ...
    const { data: existing } = await sb.from("order_payment").select("*").eq("order_id", orderId).maybeSingle();
    ...
    if (action === "create") {
      ...
      if (method !== "pix") {
        const { data: cardSnapshot, error: cardSnapshotErr } = await sb.from("order_pricing_snapshot").select("customer_total, platform_fee, service_fee_payer").eq("order_id", orderId).maybeSingle();
        ...
        let cardFinancials;
        try {
          cardFinancials = buildCardPreferenceFinancials(cardSnapshot, order.order_number ?? order.id);
        } catch (e) {
        ...
        const transparentPayerEmail = String(payload?.payer_email ?? "").trim().toLowerCase();
        ...
        const { data: transparentPreUp, error: transparentPreErr } = await sb.from("order_payment").upsert({
          order_id: orderId,
        ...
        const transparentNotificationUrl = "https://mvkfrwxgneqzvoabkaws.supabase.co/functions/v1/mp-webhook";
        const transparentIdempotencyKey = await buildCardIdempotencyKey(orderId, transparentCard.token);
        ...
        const { error: transparentPostErr } = await sb.from("order_payment").upsert({
          order_id: orderId,
          restaurant_id: order.restaurant_id,
          provider: "mercado_pago",
          payment_method: "credit_card",
          ...
          transaction_amount: Number(transparentMp?.transaction_amount ?? cardFinancials.customerTotal),
          payment_url: null,
          last_error: transparentStatus === "REJECTED" ? JSON.stringify({
        ...
        if (transparentPaymentErr) console.error("[mp-payment-intent] payments upsert failed (transparent card)", {
          orderId,
        ...
        if (order.customer_id) {
          const { data: prof } = await sb.from("customer_profiles").select("email, phone, whatsapp").eq("id", order.customer_id).maybeSingle();
          custEmail = prof?.email ?? null;
          custPhone = custPhone || prof?.phone || prof?.whatsapp || null;
          const { data: addr } = await sb.from("customer_addresses").select("cep, street, number, city, state").eq("customer_id", order.customer_id).order("is_default", {
            ascending: false
          }).limit(1).maybeSingle();
          custAddress = addr ?? null;
        }
        if (!custEmail) {
          const { data: cust } = await sb.from("customers").select("email, phone").eq("restaurant_id", order.restaurant_id).eq("phone", order.customer_phone ?? "").maybeSingle();
        ...
        const finalEmail = custEmail && /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(custEmail) ? custEmail : payloadEmail && /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(payloadEmail) ? payloadEmail : null;
      ...
      const { data: pricingSnapshot, error: pricingErr } = await sb.from("order_pricing_snapshot").select("platform_fee").eq("order_id", orderId).maybeSingle();
      if (pricingErr) throw pricingErr;
      const rawPlatformFee = pricingSnapshot?.platform_fee;
      const platformFee = Number(rawPlatformFee);
      if (rawPlatformFee === null || rawPlatformFee === undefined || rawPlatformFee === "" || !Number.isFinite(platformFee) || platformFee < 0) {
        return json({
      ...
      const ticketUrl = mp?.point_of_interaction?.transaction_data?.ticket_url ?? mp?.transaction_details?.external_resource_url ?? null;
      ...
      if (payErr) console.error("[mp-payment-intent] payments upsert failed", {
        orderId,
    ...
    // ---------- STATUS ----------
    if (action === "status") {
      if (!existing?.payment_id) return json({
