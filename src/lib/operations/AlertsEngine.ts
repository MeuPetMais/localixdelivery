...
export function buildAlerts(
  ...
  for (const c of cards) {
    ...
    if (!c.paymentApproved && c.status === "WAITING_PAYMENT" && ageMin > 10) {
      alerts.push({
    ...
    if (ageMin > LATE_THRESHOLD_MIN && !["DELIVERED", "COMPLETED", "CANCELLED"].includes(c.status)) {
      alerts.push({
    ...
    if (c.status === "OUT_FOR_DELIVERY" && c.etaMinutes && ageMin > c.etaMinutes + DRIVER_LATE_MIN) {
      alerts.push({
