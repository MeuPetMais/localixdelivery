...
describe("Chef AI live benchmark", () => {
  it("runs Luna and Gemini through the paid AI Gateway preview", async () => {
    expect(process.env.AI_GATEWAY_API_KEY).toBeTruthy();
    ...
    console.log("CHEF_LIVE_BENCHMARK_START");
    console.log(JSON.stringify({ generatedAt: new Date().toISOString(), cases: CASES.length, benchmark }));
    console.log("CHEF_LIVE_BENCHMARK_END");

    expect(benchmark.some((model) => model.successRate > 0)).toBe(true);
  }, 120000);
});
