...
export function toSnapshot(row: Row): TrackingSnapshot {
  return {
    ...
    last_accuracy: row.last_accuracy == null ? null : Number(row.last_accuracy),
    last_seen_at: (row.last_seen_at as string | null) ?? null,
    device_captured_at: (row.device_captured_at as string | null) ?? null,
    server_received_at: (row.server_received_at as string | null) ?? null,
