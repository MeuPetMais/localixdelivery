...
const CASES = [
  ...
  {
    id: "promotion",
    message: "Quero aproveitar alguma promoção hoje.",
    expected: { promotionPreferred: true },
  },
  {
    id: "category",
    message: "Quero uma pizza.",
    expected: { category: "pizza" },
  },
  {
    id: "budget_only",
    message: "Quero gastar no máximo 30 reais.",
    expected: { budgetMax: 30 },
  },
  {
    id: "open_choice",
    message: "Escolha por mim, estou com bastante fome.",
...
function evaluate(result: Awaited<ReturnType<ReturnType<typeof createGatewayIntentParser>["parse"]>>, expected: (typeof CASES)[number]["expected"]) {
  ...
  if ("noInventedCommercialFacts" in expected) {
    checks.noInventedCommercialFacts =
      ...
      (result.intent.desiredTerms?.length ?? 0) <= 2 &&
